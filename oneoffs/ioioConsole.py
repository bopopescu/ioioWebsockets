...
LEDMODE = 3  # Menu: CTRL+T
LON = 0  # Menu: CTRL+T
LOFF = 1  # Menu: CTRL+T
LEDOFF = bytearray(b'\x03\x01');
LEDON = bytearray(b'\x03\x02');
...
def get_help_text():
    return """
--- IOIOConsole.py (%(version)s) - IOIO Console control - help
---
            ...
            while self.alive:
                ...
                if menu_active:
                    if c == MENUCHARACTER or c == EXITCHARCTER: # Menu character again/exit char -> send itself
                        ...
                        sys.stdout.write('turning on LED')
                        sys.stdout.flush()
                        self.serial.write(LEDON)         # set LED on mode
                        if self.echo:
                            sys.stdout.write(LEDON)
                            sys.stdout.flush()
                    elif c == 'f':
                        sys.stdout.write('turning off LED')
                        sys.stdout.flush()
                        self.serial.write(LEDOFF)         # set LED on mode
                        if self.echo:
                            sys.stdout.flush()
                            sys.stdout.write(LEDOFF)
                    else:
